...
## 2026-07-23｜計測・通知・タイポ階層の横断改修

- GA4のクリック委譲5イベントとSPA `page_view`、主要CTA 13箇所の `data-cta` を追加。測定ID未設定時はno-op。
- Resend成功後のSlack Webhook通知を追加。envで有効化し、Slack失敗時もメール成功を維持する。
- Inter→Noto Sans JPの和欧混植、見出し5段＋余白5段の `clamp()`、4段グレー、`SectionHeading`、統一イージング、FV reveal、8件までのCSS staggerを導入。
- works 7件を単位＋比較注釈へ変更。実測済み4件、`[要記入]` 3件。リンクのないmedia-pipelineには状態説明を表示。
- `/thanks`（noindex）を追加し、送信成功時の着地点にした。
- Next.jsを15.5.21へ更新し、未使用の`@cloudflare/next-on-pages`を削除。ESLintゲートを追加。

**検証**：`npx tsc --noEmit`／`npm run lint`／`npm run build`成功。375/768/1280pxで横スクロール0・エラーオーバーレイ0、reduced motion時の非表示要素0、キーボードfocus-visibleリングを確認。`clamp()` 10件、`foreground/NN` 0種、SectionHeadingの生h2漏れ0。
